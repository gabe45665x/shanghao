...
(function () {
  ...
  /* Brand mark: TAIWAN（整個字加 em 上金色） */
  $$(".brand-word, .brand-name-zh").forEach((el) => {
    if (!el.querySelector("em")) el.innerHTML = "<em>TAIWAN</em>";
  });
  ...
  if (!$("#sidebar")) {
    ...
    side.id = "sidebar";
    side.hidden = true;
    side.innerHTML = `<p class="kicker">TAIWAN · MENU</p>
      ...
      <h2 style="margin:8px 0 16px"><em style="color:var(--accent);font-style:normal">TAIWAN</em></h2>
  ...
  if ($("#age-gate")) {
    ...
    wrap.innerHTML = `<div class="age-gate" id="age-gate" role="dialog" aria-modal="true">
      <div class="age-card">
        <img src="img/logo-mark.jpg?v=s2" alt="TAIWAN" width="96" height="96" class="logo-mark">
        <p class="kicker">PROTOCOL / 18+</p>
        <h2>進入<em style="color:var(--accent);font-style:normal">TAIWAN</em>協議</h2>
        <p>本站限年滿 18 歲者瀏覽。未滿 18 歲請離開。</p>
        <label class="age-check"><input id="age-confirm" type="checkbox"> 我已年滿 18 歲，並接受條款。</label>
        <div class="age-actions">
          <button class="btn btn-solid" type="button" id="age-enter">進入TAIWAN</button>
  ...
  /* Listings from data.js — keep TAIWAN卡片／規格卡結構 */
  ...
  registerForm?.addEventListener("submit", (e) => {
    ...
    if (cap && cap.toUpperCase() !== "TAIWAN") { toast("驗證碼不正確（示範：TAIWAN）"); return; }
    sessionStorage.setItem("xinse-user", user);
    toast("帳號已建立");
    const next = new URLSearchParams(location.search).get("next") || "account.html";
    setTimeout(() => { location.href = next; }, 400);
  ...
  if (profileRoot && window.XINSE_DATA) {
    ...
    if (!p) {
      ...
      document.title = `${p.name} ${p.protocol} · TAIWAN`;
